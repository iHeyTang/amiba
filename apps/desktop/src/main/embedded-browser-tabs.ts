...
export class BrowserTabIndex<E extends BrowserTabRecord> {
  ...
  activate(entry: E): void {
    this.globalActiveKey = entry.key;
    if (entry.sessionId) this.activeKeyBySession.set(entry.sessionId, entry.key);
  }
  ...
  delete(key: string): void {
    ...
    if (entry?.sessionId && this.activeKeyBySession.get(entry.sessionId) === key) {
      const replacement = this.firstLiveKey(entry.sessionId);
      if (replacement) this.activeKeyBySession.set(entry.sessionId, replacement);

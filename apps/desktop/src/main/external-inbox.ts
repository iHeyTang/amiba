 ...
 *      `{ "text": "..." }\n`. macOS + Linux only; Windows skips it
 *      cleanly (no socket, no error).
 *
 * Both funnel into `deliverPrompt`, which mirrors HomeView's hand-off
 * format: write `{ text, ts }` under `home.pendingPrompt` and summon the
 * window. The renderer's existing watcher (see App.tsx) flips to the
 * chat view, and SidePanelView's mount drain picks the prompt up and
 * auto-sends it.
 */
...
/** Write to the same key HomeView writes, then raise the window. */
export async function deliverPrompt(text: string, summon: Summon): Promise<void> {
 ...
 * per-socket buffer so partial reads don't split a JSON object.
 *
 ...
 */
export async function startUnixSocketInbox(summon: Summon): Promise<net.Server | null> {
...
function handleInboxLine(line: string, summon: Summon): void {
  ...
  void deliverPrompt(text, summon)
}

export async function stopUnixSocketInbox(server: net.Server | null): Promise<void> {
  if (!server) return

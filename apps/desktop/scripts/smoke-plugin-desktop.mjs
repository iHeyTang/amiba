...
try {
  ...
  if (process.argv.includes("--compat")) {
    ...
    if (process.argv.includes("--input-state")) {
      ...
      assert.deepEqual({...fileStorageResult,ref:{...fileStorageResult.ref,attachmentId:'digest'}},{receiptResolved:true,rollbackPreserved:true,committedRetired:true,ref:{attachmentId:'digest',name:'compat-host-file.bin',bytes:5},sameDigest:true,data:'AP8qgAc=',imagePreserved:true,pathHasName:true});

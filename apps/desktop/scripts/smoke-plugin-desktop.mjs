import { smokeCordisBusiness } from "./cordis-business-smoke.mjs";
...
if (process.argv.includes("--cordis-only") && !["--author", "--compat", "--cordis-business"].every(flag => process.argv.includes(flag))) {
  throw new Error("--cordis-only requires --author --compat --cordis-business");
}
...
try {
  ...
  // The product shell can mount before the desktop development endpoint is ready.
  const { discoverDevelopmentEndpoint } = await import("../../cli/dist/lib/development-connection.js");
  await wait(async () => {
    try { await discoverDevelopmentEndpoint([home]); return true; } catch { return false; }
  });
  ...
  if (process.argv.includes("--cordis-only")) {
    ...
    await wait(() => evaluate("!!document.querySelector('[data-composer-card] [contenteditable=true]')"));
    await smokeCordisBusiness({ profile, evaluate, wait, call });
  } else if (process.argv.includes("--compat")) {

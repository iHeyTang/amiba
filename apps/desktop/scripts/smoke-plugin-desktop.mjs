...
try {
  ...
  const clientSource = version => `import {createElement} from "react";${process.argv.includes("--compat") ? "export const inject = [\"slots\", \"settingsScope\", \"layout\", \"sessions\", \"sessionLogDownload\", \"inputTriggers\", \"composerInputs\", \"composerImages\"];" : ""}export function apply(ctx: any) { (window as any).__probeCreateElement=createElement; (window as any).__probeCtx=ctx; ctx.effect(() => { const node=document.createElement('div');node.id='amiba-plugin-probe';node.textContent='client-${version}';document.body.append(node);return ()=>node.remove(); }); }`;

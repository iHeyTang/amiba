...
由 desktop spawn + 监管(desktop 是进程主管),把 Hermes core(以及已装的
`hermes-x-plugin-integrations`)的能力暴露成本机客户端(浏览器扩展、桌面渲染端)
能消费的 HTTP。默认监听 `127.0.0.1:9394`(用 `HERMES_BACKPLANE_PORT` 覆盖)。
...
> 对比:`hermes-x-plugin-browser-tools`(注册 `my_browser_*` tools)、
> `hermes-x-plugin-integrations`(注册 `hermes integration` CLI + skills)**仍是真
> plugin** —— 它们确实给 agent 加能力。本仓不碰 agent 能力面,是独立 server。
...
| Repo | 角色 |
|---|---|
| **this repo** | 本地 HTTP server：`/hermes/*` + integrations gateway（`/integrations/<name>/search`、`/hermes/mention-resources`、`/hermes/integrations*`） |
| [hermes-x-plugin-integrations](https://github.com/iHeyTang/hermes-x-plugin-integrations) | integration 协议 + `hermes integration` CLI + 进程内能力注册表（**本仓的 integrations gateway 读它**；没装则 gateway 优雅降级） |
| [hermes-x-plugin-browser-tools](https://github.com/iHeyTang/hermes-x-plugin-browser-tools) | 给 agent 的 browser 工具（screenshot / navigate / inbox 等），通过 WS bridge 连扩展 |
| [hermes-my-browser-extension](https://github.com/iHeyTang/hermes-my-browser-extension) | Chrome 扩展前端，调本插件的 `/hermes/*` 端点 |

本仓本身就提供 `/hermes/*` 全套 + `/integrations/*`(它自己加载
...
| `integrations_gateway` | `/hermes/integrations*` + `/integrations/<name>/search` + `/hermes/mention-resources` | 适配 `hermes-x-plugin-integrations` 注册表的网关（**不是** agent tool；没装那插件就降级） |

### integrations gateway —— `/integrations/<name>/search` 等

集成框架本身（loader / manager / `hermes integration` CLI）**不在本仓** —— 它住在
独立的 [`hermes-x-plugin-integrations`](https://github.com/iHeyTang/hermes-x-plugin-integrations)。
集成是 **HTTP-agnostic 的纯能力**（只声明一个 `search` 能力 + manifest，不碰 aiohttp）。
本仓只有一个**适配层**（`runtime/features/hermes_proxy/integrations_gateway/`），进程内读
那个插件的注册表，把它适配成 HTTP：
...
- `GET /hermes/mention-resources` —— 聚合各集成的 `mention_resources`，给 composer 的
  `@` 提及；
- `/hermes/integrations*` —— lifecycle admin，delegate 给那个插件的 manager（供
  `hermes integration` CLI 热重载用）。

没装 `hermes-x-plugin-integrations` 时，这几条路由优雅降级（空注册表 / 503），backplane
其余部分不受影响。HTTP 这层完全由本仓负责，集成自己一点 HTTP 都不沾 —— 详见那个
插件的 README。
...
- **自己加载集成**：启动时 `server.py:_load_integrations()` 调
  `hermes_plugin_integrations.loader.load_all()` 把 `~/.hermes/integrations/`
  载进**本进程**的注册表(集成插件在 gateway 进程,跨进程读不到,所以自己加载)。
  集成插件没装就空注册表降级。
...
- 集成怎么写 / `hermes integration` CLI / resolver skill —— 见
  [`hermes-x-plugin-integrations`](https://github.com/iHeyTang/hermes-x-plugin-integrations)
...
（集成框架 / `hermes integration` CLI / resolver skills 都在
`hermes-x-plugin-integrations`，不在本仓。）

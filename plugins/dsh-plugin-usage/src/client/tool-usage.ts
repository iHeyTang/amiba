/**
 * Client-side aggregation for the Tools activity view.
 *
 * Faithful port of the tool-meter extension's main-process IPC handlers
 * (meter.summary / meter.recent / meter.heatmap), previously hosted in
 * `packages/ui/src/usage/tool-usage.ts` against a desktop-injected
 * reader. Now the reader is the plugin's own Typert Remote
 * (`amibaUsage/readToolActivity`); it returns raw per-day invocation
 * rows (most recent day first, exactly the requested count) plus the
 * lifetime aggregate, and everything here stays pure client-side
 * crunching.
 */
...
/** Remote-backed ledger read the client half injects into the view. */
export type ToolActivityReadFn = (
  days: number,
) => Promise<ToolActivityReadResult>;
...
export async function fetchToolSummary(
  read: ToolActivityReadFn,
): Promise<MeterSummary> {
  const todayKey = toolActivityDayKey(Date.now());
  const { days: recentRaw, lifetime } = await read(7);
...
export async function fetchToolHeatmap(
  ...
  const { days: rawDays } = await read(cappedWeeks * 7);

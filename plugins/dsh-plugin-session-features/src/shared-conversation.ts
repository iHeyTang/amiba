...
export function sharedConversationSeed(sessionId: string, origin: ConversationOrigin): SessionEvent[] {
  return [{ type: SHARED_CONVERSATION_EVENT, seq: SessionSeq(0), time: Date.now(), ignorable: true, data: { sessionId, origin } }];
}
/** Unlike a plugin role, a privacy boundary survives copied/forked history. */
export function sharedConversationBinding(session: { snapshotEvents(): readonly SessionEvent[] }): SharedConversationBinding | undefined {
  const events = session.snapshotEvents().filter((event) => event.type === SHARED_CONVERSATION_EVENT);
...
export function installSharedConversationAccess(ctx: Context): () => void {
  ...
  try {
    ...
    disposers.push(ctx.systemPrompt.section({
      name: PERSONA_PREFIX_SECTION, order: ctx.systemPrompt.getSectionOrder("DEPLOYMENT_PERSONA_PREFIX"), complete: true,

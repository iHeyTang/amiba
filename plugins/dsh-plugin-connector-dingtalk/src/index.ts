...
export class DingtalkPersonalRemoteService extends TypertRemoteService {
  constructor(
    private setupContext: Context,
    private personal: DingtalkPersonalService,
  ) {
    super(setupContext, "amibaDingtalkPersonalRemote", {
...
export function apply(ctx: Context): void {
  ...
  const personal = new DingtalkPersonalService(
    ctx.amibaConnectors.accounts("dingtalk"),
  );
  ctx.effect(() => () => personal.dispose());
  new DingtalkPersonalRemoteService(ctx, personal);
  ctx.inject(["amibaResources"], (resources) => {
    resources.effect(() =>
      resources.amibaResources.register(createDingtalkResourceSource(personal)),
    );
  });
}

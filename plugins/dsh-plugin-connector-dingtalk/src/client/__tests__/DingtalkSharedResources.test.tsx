...
import { DingtalkSharedResources } from "../DingtalkSharedResources.js";
...
it("searches without granting access, then saves explicit choices to the current chat", async () => {
 const f = fixture();
 render(<DingtalkSharedResources management={f.management} chatKey="group-one" grants={[]} saved={f.saved} />);
...
it("revokes an existing grant without needing a resource search", async () => {
 const f = fixture();
 render(<DingtalkSharedResources management={f.management} chatKey="group-two" grants={[resource]} saved={f.saved} />);
...
it("retains the editable selection when saving fails and does not report success", async () => {
 ...
 render(<DingtalkSharedResources management={f.management} chatKey="group" grants={[resource]} saved={f.saved} />);
 fireEvent.click(screen.getByRole("checkbox", { name: "Project notes" }));
 fireEvent.click(screen.getByRole("button", { name: "Save shared resources" }));

...
export function DingtalkConversationSettings({ host }: { host: ConnectSettingsHost }) {
  ...
  return <section className="space-y-4" aria-label={t("conversations.title")}>
    ...
    {!management ? <p className="text-sm text-muted-foreground">{t("conversations.unavailable")}</p> : <>
      ...
      {view ? <div className="space-y-4">
        ...
        {view.access === "shared" ? <DingtalkSharedResources key={key} management={management} chatKey={key} grants={view.sharedResources} saved={setView} /> : null}

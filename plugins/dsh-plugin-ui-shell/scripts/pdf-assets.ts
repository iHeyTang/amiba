...
/** Losslessly compress each resource at build time; retain same-version licenses. */
export function pdfBuildData(): { assets: string; banner: string } {
  ...
  return {
    assets: JSON.stringify(Object.fromEntries(groups.map(([kind, folder]) => [kind, Object.fromEntries(readdirSync(join(root, folder)).filter(name => !name.startsWith('LICENSE')).sort().map(name => [name, gzipSync(readFileSync(join(root, folder, name)), { level: 9 }).toString('base64')]))]))),

...
it("hands off exact call IDs and acknowledges only the request that was rendered", () => {
  ...
  expect(result.current.owner.inspect).toEqual({ callId: "nested-call" });
  const oldAck = result.current.owner.onInspectDone;
  act(() => result.current.inspectCall!("new-call"));
  act(oldAck);
  expect(result.current.owner.inspect).toEqual({ callId: "new-call" });
  act(result.current.owner.onInspectDone);
  expect(result.current.owner.inspect).toBeNull();

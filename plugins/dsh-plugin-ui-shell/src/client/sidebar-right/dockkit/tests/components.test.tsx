...
import type { DockIntents } from '../contract/adapter.js'
import type { PaneId, TabId } from '../contract/types.js'
import { DockController } from '../engine/controller.js'
import { applyOp } from '../engine/operations.js'
import { FLOAT_DEFAULT_SIZE, FLOAT_MIN_SIZE } from '../engine/constraints.js'
import { floatRectAt } from '../engine/geometry.js'
import { DockSurface, type DockSurfaceProps } from '../components/DockSurface.js'
import type { TabMenuExtras } from '../contract/adapter.js'
import { FloatLayer, type FloatLayerProps } from '../components/FloatLayer.js'
import { dockPaneIds, getPane } from '../engine/tree.js'
import { TEST_LABELS, asPane, asTab, fileTab, seededController } from './fixtures.js'

// The repository's jsdom predates PointerEvent; preserve real mouse coordinates
// and pointer identity so gesture tests exercise the component's ownership rules.
beforeEach(() => {
...
describe('DockSurface', () => {
  ...
  it('hides blocked split controls when opted in and restores them when capacity returns', () => {
    ...
    expect(intents.splitPane).toHaveBeenCalledTimes(1)
    expect(intents.splitPane).toHaveBeenCalledWith(dockPaneIds(state)[0])
  })
  ...
  it('offers close only for tabs the embedder allows', () => {
    ...
    fireEvent.click(screen.getByRole('menuitem', { name: TEST_LABELS.closeTab }))
    expect(intents.closeTab).toHaveBeenCalledTimes(1)
    expect(intents.closeTab).toHaveBeenCalledWith(allowed)
  })
  ...
  it.each([false, true])('hides the popup when an extras component renders no content inside a wrapper: %s', (wrapped) => {
    ...
    style.textContent = readFileSync(resolve('src/client/sidebar-right/dockkit/components/dockkit.module.css'), 'utf8')
...
describe('horizontal workbench drops', () => {
  it.each([
    ...
    expect(intents.dropTab).toHaveBeenCalledTimes(1)
    expect(intents.dropTab).toHaveBeenCalledWith(fileTabId, second, zone)
  })
  ...
  it('uses the entire other pane for moving once the split budget is spent', () => {
    ...
    expect(intents.dropTab).toHaveBeenCalledTimes(1)
    expect(intents.dropTab).toHaveBeenCalledWith(fileTabId, second, 'center')
  })

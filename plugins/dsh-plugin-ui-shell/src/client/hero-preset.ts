...
export function createHeroPreset(loadRoster: () => ReturnType<AgentPresetsAdapter['list']>) {
  ...
  const injected: AgentPresetSeatInjected = {
    ...
    load: () => pending ?? (pending = (async () => {
      try {
        const { presets } = await loadRoster();
        const state = store.getSnapshot();
        store.set({ ...state, options: presets, error: null, current: staged ? state.current : presets.find(p => p.isDefault)?.id ?? presets[0]?.id ?? '' });
  ...
  return { ...injected, store, reset() { staged = false; void injected.load(); } };
}

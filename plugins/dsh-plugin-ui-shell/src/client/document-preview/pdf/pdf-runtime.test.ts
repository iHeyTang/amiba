...
import type { PdfDocument, PdfSession } from './document.js'
...
describe('PDF Worker lifecycle', () => {
  it('starts a real module-worker port before calling getDocument with complete bytes and local assets', async () => {
    ...
    workers[0]!.ready()
    const options = await h.entered.promise as { data: Uint8Array; BinaryDataFactory: unknown }
    ...
    expect(revokeURL).toHaveBeenCalledTimes(1); expect(revokeURL).toHaveBeenCalledWith('blob:pdf-worker')
  })
  ...
  it('terminates after messageerror even when the failure callback throws', async () => {
    ...
    await h.session.dispose()
    expect(log).toHaveBeenCalledTimes(1); expect(log).toHaveBeenCalledWith('[pdf] failure callback threw', callbackError)
    expect(workers[0]!.terminate).toHaveBeenCalledOnce()
    expect(revokeURL).toHaveBeenCalledOnce()
  })
  ...
  it('logs a library teardown rejection after releasing the bridge, Worker, and Blob', async () => {
    ...
    expect(log).toHaveBeenCalledTimes(1); expect(log).toHaveBeenCalledWith('[pdf] cleanup failed', failure)
    expect(api.destroyBridge).toHaveBeenCalledOnce()
    expect(workers[0]!.terminate).toHaveBeenCalledOnce()
    expect(revokeURL).toHaveBeenCalledOnce()
  })

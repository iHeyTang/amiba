...
import type { HighlightSpan, StreamingHighlightFrame } from './highlight.js'
...
export function CodeBlock({ code, lang, streaming, className, contentRef, lineNumbers = false, copyLabel, copiedLabel }: CodeBlockProps) {
  ...
  const streamedBody = useMemo(() => {
    ...
    const finalAppend = streaming !== true && !settledRef.current
      && lineCacheRef.current !== null && lineCacheRef.current.lang === lang
      && trimmed.startsWith(lineCacheRef.current.code)
    if (streaming !== true && !finalAppend) {
    ...
    if (previous?.frame === frame && previous.code === trimmed && previous.lang === lang) {
      settledRef.current = streaming !== true
      return previous.body
    }

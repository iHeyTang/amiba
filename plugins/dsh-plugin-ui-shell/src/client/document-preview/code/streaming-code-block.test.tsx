...
import { StreamingHighlightSession } from './highlight.js'
import { markdownLabels } from './labels.client.js'
...
it('retains completed lines when the final document page appends text and settles together', () => {
  const prefix = Array.from({ length: 40 }, (_, i) => `const n${i} = ${i};`).join('\n')
  const view = render(<CodeBlock code={prefix} lang="ts" streaming {...LABELS} />)
  const first = view.container.querySelector('pre .line')
  const scrollport = view.container.querySelector('[data-code-block-content]')
  const final = prefix + '\nconst final = 42;'
  view.rerender(<CodeBlock code={final} lang="ts" streaming={false} {...LABELS} />)
  expect(view.container.querySelector('pre .line')).toBe(first)
  expect(view.container.querySelector('[data-code-block-content]')).toBe(scrollport)
  expect(view.container.querySelector('pre')?.textContent).toBe(final)
})

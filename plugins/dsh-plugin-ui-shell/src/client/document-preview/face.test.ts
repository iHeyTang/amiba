...
import type { DocumentFileBytes, ReadDocumentBytes, ReadWorkspaceFilePage } from './rpc.js'
import { hostFileOf } from './rpc.js'
import { createTextStore } from './store.js'
import { ABSOLUTE_PATH, FILE, PATH, SESSION, failure, page } from './fixtures.js'
...
describe('textFace', () => {
  ...
  it('forgets the tab when its record ends, once, however many reads armed it, and writes nothing afterwards', async () => {
    ...
    controller.abort()
    expect(forget).toHaveBeenCalledTimes(1); expect(forget).toHaveBeenCalledWith(TAB_1)
  ...
  it('loads native complete bytes with only the tab signal', async () => {
    ...
    expect(bytes).toHaveBeenCalledTimes(1); expect(bytes).toHaveBeenCalledWith(FILE, controller.signal)
  ...
  it.each([PATH, ABSOLUTE_PATH, 'C:/w/notes.md', '//host/share/notes.md'])('uses the addressed Session for %s in both reading modes', async (path) => {
    ...
    expect(first.read).toHaveBeenCalledTimes(1); expect(first.read).toHaveBeenCalledWith(firstFile.sessionId, firstFile.path, 1, first.controller.signal)
    expect(second.read).toHaveBeenCalledTimes(1); expect(second.read).toHaveBeenCalledWith(secondFile.sessionId, secondFile.path, 1, second.controller.signal)
    ...
    expect(first.bytes).toHaveBeenCalledTimes(1); expect(first.bytes).toHaveBeenCalledWith(firstFile, first.controller.signal)
    expect(second.bytes).toHaveBeenCalledTimes(1); expect(second.bytes).toHaveBeenCalledWith(secondFile, second.controller.signal)

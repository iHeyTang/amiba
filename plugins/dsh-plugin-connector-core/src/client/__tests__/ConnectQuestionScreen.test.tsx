...
describe("ConnectQuestionScreen", () => {
  it("opens the prefilled provider's screen and answers with only the connect id", async () => {
    ...
    registry.register("lark", {
      component: ({ host }) => (
        <button
          type="button"
          onClick={() => host.done({ id: "c1", provider: "lark" } as never)}
        >
          finish:{host.prefill?.name}
        </button>
      ),
    });
    const p = props(
      JSON.stringify({ provider: "lark", name: "飞书助手" }),
      registry,
    );
    render(<ConnectQuestionScreen {...p} />);
    await userEvent.click(await screen.findByText("finish:飞书助手"));
    expect(p.respond).toHaveBeenCalledWith([
      { id: "amiba.connect-wizard", selected: [], custom: "c1" },
    ]);
  });
  it("shows the chooser when no provider was prefilled, then the picked provider's screen", async () => {
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: /飞书 \/ Lark/ }),
    );
  ...
  it("shows the owner's delivery error alongside a load failure", async () => {
    ...
    expect(
      await screen.findByText(
        /该连接已不存在。|That connect no longer exists\./,
      ),
    ).toBeInTheDocument();
  ...
  it("renders inside the shared composer dock sheet", async () => {
    const { container } = render(
      <ConnectQuestionScreen {...props(undefined)} />,
    );
    ...
    expect(
      container.querySelector("[data-connect-question-screen]"),
    ).toBeNull();
  });

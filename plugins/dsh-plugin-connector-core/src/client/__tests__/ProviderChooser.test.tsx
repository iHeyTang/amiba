...
describe("ProviderChooser", () => {
  it("lists only registered providers and picks on click", async () => {
    ...
    registry.register("lark", { component: () => null, tagline: "扫码或填写凭证" });
    const onPick = vi.fn();
    render(<ProviderChooser onCancel={vi.fn()} onPick={onPick} providers={providers} registry={registry} subtitle="选择平台" title="添加连接" />);
  ...
  it("shows the hint in the footer and cancels", async () => {
    ...
    render(<ProviderChooser hint="没有你要的平台？" onCancel={onCancel} onPick={vi.fn()} providers={providers} registry={createConnectWizardRegistry()} title="添加连接" />);

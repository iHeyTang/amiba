...
import { LarkPersonalService, personalAccess } from "./personal.js";
...
export class LarkPersonalRemoteService extends TypertRemoteService {
  constructor(
    ...
    super(ctx, "amibaLarkPersonalRemote", { namespace: "amibaLarkPersonal" });
  }
  @Remote async manage(input: PersonalRequest): Promise<PersonalResult> {
    ...
    switch (input.action) {
      case "status":
        return { status: await this.personal.status(id) };
      case "begin":
      ...
      case "model-access":
        ...
        return {
          status: await this.personal.setModelAccess(id, input.allowed),
        };
    }
  }
}

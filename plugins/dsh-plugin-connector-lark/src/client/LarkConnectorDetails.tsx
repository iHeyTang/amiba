import { Bot, QrCode, ShieldCheck, Wrench } from "lucide-react";
...
export function LarkConnectorDetails() {
  const { t } = usePluginT(larkI18n);
  return (
    <div className="space-y-8" data-provider-details="lark">
      <p className="max-w-3xl text-sm leading-7 text-muted-foreground">
        {t("options.connect.dsh.lark.detail.summary")}
      </p>
      <section aria-labelledby="lark-capabilities" className="space-y-3">
        <h2
          className="text-[15px] font-semibold tracking-[-0.01em]"
          id="lark-capabilities"
        >
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block text-sm font-medium">
                  {t(`options.connect.dsh.lark.detail.${key}.title`)}
                </span>
                <span className="mt-0.5 block text-[13px] leading-5 text-muted-foreground">
                  {t(`options.connect.dsh.lark.detail.${key}.description`)}
                </span>
              </span>
            </div>
          ))}

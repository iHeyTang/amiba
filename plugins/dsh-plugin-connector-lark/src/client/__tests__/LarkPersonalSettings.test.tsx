...
const capabilities = ["search:docs:read", "docx:document:readonly"].map(
...
function fixture(initial: any = unauthorized) {
  ...
  const remote = { manage } as unknown as LarkPersonalRemote;
  return { manage, remote };
}
describe("Lark connection experience", () => {
  it("opening existing settings preserves the user's paused AI access", async () => {
    vi.useFakeTimers();
    const h = fixture(authorized());
    render(<LarkPersonalSettings host={host} remote={h.remote} />);
    await flush();
    expect(
  ...
  it("cancels a flow that arrives after closing the page", async () => {
    ...
    const view = render(<LarkPersonalSettings host={host} remote={h.remote} />);
    await flush();
    fireEvent.click(
      screen.getByRole("button", {
        name: "Complete connection and enable AI",
  ...
  it("only shows the completion action when document access is ready", async () => {
    ...
    render(
      <LarkPersonalSettings host={host} remote={h.remote} onDone={done} />,
    );
    await flush();
    fireEvent.click(
...
it("automatically continues a newly created connection to the platform scan", async()=>{vi.useFakeTimers();const h=fixture();render(<LarkPersonalSettings host={host} remote={h.remote} autoStart/>);await flush();expect(h.manage).toHaveBeenCalledWith({action:"begin",connectionId:"a"});expect(h.manage.mock.calls.some(c=>c[0].action==="complete-connection")).toBe(false);await flush(5000);expect(h.manage).toHaveBeenCalledWith({action:"complete-connection",connectionId:"a"});});
it("does not claim completion while work tools are still starting",async()=>{vi.useFakeTimers();const h=fixture({...authorized(true),work:"pending"});render(<LarkPersonalSettings host={host} remote={h.remote} onDone={vi.fn()}/>);await flush();expect(screen.queryByRole("button",{name:"Done, start using AI"})).not.toBeInTheDocument();expect(screen.getAllByText("Finishing connection setup…").length).toBeGreaterThan(0);});

...
describe("Amiba companion behavior", () => {
  ...
  it("does not celebrate restored history or celebrate forever", () => {
    ...
    expect(companionPose("completed", 0).action).toBe(
      hasCompanionActions ? "victory-hop" : "expression-completed",
    );
    expect(companionPose("completed", 12).state).toBeLessThan(18);
    expect(companionPose("failed", 12).state).toBeLessThan(18);
  });
  it.skipIf(!hasCompanionActions)(
    "uses different body choreography for thinking and waiting",
    () => {
...
it.skipIf(!hasCompanionActions)(
  "plays four different authored actions for every lifecycle scene on all rigs",
  () => {
    const scenes = [
      "idle",
      "loading",
...
it.skipIf(hasCompanionActions)(
  "published rigs use valid idle body poses and scene expressions",
  () => {
    expect(companionPose("thinking", 1)).toMatchObject({
      state: 0,
      expression: 1,
      followsPointer: false,
    });
    expect(companionPose("waiting", 1)).toMatchObject({
      state: 0,
      expression: 11,
      followsPointer: true,
    });
    expect(companionPose("completed", 0).expression).toBe(3);
    expect(companionPose("completed", 30)).toMatchObject({
      state: 0,
      expression: -1,
      followsPointer: true,
    });
    expect(companionPose("thinking", 1, false, true)).toEqual(
      companionPose("thinking", 4, false, true),
    );
  },
);

it("uses native rig actions for onboarding without a container animation", () => {
  for (const [scene, action] of [
    ["welcome", "happy-dance"],
    ["cheering", "victory-hop"],
    ["impatient", "brake-rock"],
  ] as const) {
    expect(companionPose(scene, 0)).toMatchObject({
      action,
      followsPointer: false,
    });
    expect(companionPose(scene, 0).state).toBeGreaterThanOrEqual(14);
    expect(
      companionPose(scene, 0).state - companionPose(scene, 0, true).state,
    ).toBe(6);
  }
});

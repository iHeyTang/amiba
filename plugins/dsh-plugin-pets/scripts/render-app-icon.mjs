...
const appIcon = prefix + svg.outerHTML.replace(/mofli-[a-f0-9-]{36}/g, 'amiba-icon');
await writeFile(new URL('../../../apps/desktop/resources/icon.svg', import.meta.url), appIcon);

// Studio is served straight from the upstream package, which declares no
// icon of its own; embed the same tile so its tab never shows a blank glyph.
await writeFile(new URL('../src/studio-icon.generated.ts', import.meta.url),
  '/** Generated by plugins/dsh-plugin-pets/scripts/render-app-icon.mjs.\n' + license + '*/\n' +
  `export const studioIconSvg = ${JSON.stringify(appIcon)}\n`);

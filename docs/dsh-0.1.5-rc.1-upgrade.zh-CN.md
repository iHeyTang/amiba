...
基线：PR #16 已合并，main 提交 `092d11c07b578911eb0e165e0869b1c94a0606e6`；已同步至 main `13315680`，保留后续布局、浏览器和供应商配置调整。目标是 npm 实际发布的 `@deepseek-ai/dsh@0.1.5-rc.1`，不是固定源码 c291e796，也不是 next 的 rc.2。
...
- 附件相关运行时 51 项、UI 12 项、输入触发/命令流程 33 项及供应商界面 11 项测试通过；运行时及桌面生产类型检查、插件构建、架构检查通过。

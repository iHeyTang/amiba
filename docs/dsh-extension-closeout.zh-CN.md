...
> 归档后按用户要求完成了主聊天普通文件模型发送、文件基础显示、冷会话上传恢复接线。最新状态和验证边界见 [归档后的收口记录](dsh-extension-compatibility-archive.zh-CN.md)；下文早期的“原输入器模型文件仍待完成”等描述已被覆盖。

> 2026-09-14：按用户要求停止继续实现，已阶段归档。当前剩余事项以 [阶段归档](dsh-extension-compatibility-archive.zh-CN.md) 为准；下文保留历史目标与逐轮进展，不再作为继续自动推进的指令。

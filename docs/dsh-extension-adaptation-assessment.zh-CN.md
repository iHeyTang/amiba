...
> 2026-09-14 归档后补齐：主聊天原输入器的普通文件模型发送已通过本地测试模型整轮及刷新验证；基础文件徽标已覆盖实时/历史，冷会话上传已接 Host lookup。当前剩余项以 [阶段归档](dsh-extension-compatibility-archive.zh-CN.md) 为准，本文历史进展中的旧待办不重复计入。
...
本表逐项覆盖初次审计两个官方源码版本的 UI 插槽名称并集：旧版固定提交 `47f94385` 为 42 个，新版固定提交 `c291e796` 为 61 个，按名称合并去重为 64 个。64 不是单一官方版本的插槽总数，也不是插件数量或全部 Host/服务扩展接口的数量；同名插槽的契约还可能随版本变化。基线为实际安装的 0.1.1-rc.2 包；新版名称来自初次审计固定提交 c291e796，仍需逐个迁移验证，不代表已支持所有新版插件。初次报告将本地源码当作 npm rc.2 的地方已发现不一致，尤其 settings.plugin.item，不能继续沿用旧结论。
...
| 模型请求转换 | 按固定新版契约将结构化文件转换为文件名、大小、只读路径文本；递归覆盖工具结果，原历史不变，与原图片策略兼容；真实 LLM 调度及 Host 路径验证通过 | session.prompt 接收/回滚/重试、文件请求 token 计量和历史 API 读回已验证；原输入器模型文件发送、重复请求确认、队列及完整历史展示/恢复仍待接入 |

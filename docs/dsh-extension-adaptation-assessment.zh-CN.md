...
| 15 | `conversation.input.attachments` | rc.2 已有契约，可以适配，尚未完成 | 实际 npm rc.2 已注册此插槽。官方描述包含浏览器 File、草稿 ID 和 previewUrl；原上传、移除与命令成功消费的浏览器注册已完成桌面验证；官方草稿反向添加、完整 imageIds 和队列恢复仍待接通，不能混用 Host staging ID。 |

...
| 官方完整 useInput/inputActions | 尚不能宣称全部支持 | inputActions 已通过 sessions.provide 连接原输入器的草稿、图片和提交操作；尚未挂载的输入器会明确拒绝官方草稿写入。原生草稿已用 v2 分别保存普通文字和真实引用节点，形似引用 token 的普通文字在切换和 renderer 重载后不会变成引用；旧 v1 草稿仍按原含义读取。完整引用身份、显式清空、旧异步读写隔离及同文不同节点类型的过期解析取消均已验证；原生排队条目现分别保留模型文本和原始节点，重新编辑、立即发送的再次解析、发送准备拒绝后的引用恢复及重试已通过桌面验证；整页刷新后的队列恢复和抢先发送也已实测。刷新后执行中会话的完整本地运行状态基线仍需补齐。官方 useInput、离屏 inputActions、图片状态和完整官方队列语义仍需继续适配，不能宣称完整支持。 |

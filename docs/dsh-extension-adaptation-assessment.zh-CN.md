...
| 14 | `conversation.hero.workspace.directoryFlow` | 可以适配，待实现 | 接入打开、选择、取消、错误生命周期，并保持现有目录初始值及写入路径。 |
...
| 44 | `settings.plugin.item` | 可以适配，卡片已验证 | 真实 rc.2 是按 Host namespace 分派的 keyed 槽；官方自带的三种配置表单仍待接入。 |
...
| 61 | `sidebar.workspaces.directoryFlow` | 可以适配，待实现 | 目录选择流程可以复用；不能产生第二个工作区状态源。 |

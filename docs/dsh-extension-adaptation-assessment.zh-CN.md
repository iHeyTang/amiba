...
| 14 | `conversation.hero.workspace.directoryFlow` | 已接入，首页插件实测通过 | 打开、选择、取消、重开及旧回调隔离已实测；保留首页路径写入。默认原生交互及浏览器选择器样式另待验证。 |
...
| 61 | `sidebar.workspaces.directoryFlow` | 已接入，项目操作实测通过 | 真实插件创建项目、取消添加、添加目录及会话路径绑定均通过；保留项目 ID 和原有路径规范化。默认原生及浏览器选择器另待验证。 |

...
| 原生队列的共享数据源 | 已接入，桌面回归通过 | 同一渲染器按存储适配器和会话共用队列源，保留原 pendingQueue 键和记录格式；原界面订阅该源。首次读取期间的新增项与旧记录合并，写入串行，迟到读取不覆盖较新的存储通知；清空面板与删除队列分开。离屏排队和后台自动出队尚未接入，这也不是 Host inbox 的原子队列服务。 |
...
| 离屏提交所需的引擎接收确认 | 已接入后台发送器，标准输入事务待接 | submitWithReceipt 区分 Host 已接收、准备/明确 RPC 拒绝、发送后结果未确认；开始事件和模型回合结束都不替代接收回执。后台发送器及标准离屏命令、草稿事务已使用真实回执，完整队列对齐仍待完成；不能直接调用 session.prompt 绕过这些流程。 |

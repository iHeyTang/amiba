...
| 54 | `sidebar.right.pane.tab` | 入口已接入，桌面验证通过 | 已接入可选工作区面板，真实会话 store、tabInfo、重复导航、收起及类型卸载回退已通过桌面验证；官方 main 隔离、浮动/全屏恢复和实际会话往返已实测；会话身份对齐后绑定，旧标签回调仍操作原会话。原生工作区其他隐藏路径、收起正文局部状态、窄屏、完整第三方插件与 Web 仍待验证。 |

...
| 模型请求转换 | 按固定新版契约将结构化文件转换为文件名、大小、只读路径文本；递归覆盖工具结果，原历史不变，与原图片策略兼容；真实 LLM 调度及 Host 路径验证通过 | session.prompt API 的 receipt 接收、失败回滚和重试已接通；原输入器模型文件发送、队列、token 统计及完整历史展示/恢复仍待接入 |

...
| 6 | `conversation.chat.turnTail` | 桌面接入已验证，Web 待验证 | 使用真实回合与结束序号；支持有回复、仅工具和空回合的位置。官方产出文件行、文件打开及目录 IPC 已验证；常规结束回复、折叠过程正文和思考标记清理后的文件名链接已通过桌面验证；中断回复、历史重开和自定义 Markdown 变换仍在核对。 |

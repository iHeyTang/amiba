...
describe("measureComposerDockTargetHeight", () => {
  it("uses the editor destination instead of an intermediate animation frame", () => {
    ...
    expect(measureComposerDockClearance(dock)).toBe(150);
  });

  it("does not undercount a painted composer constrained by editor min-height", () => {
    const dock = document.createElement("footer");
    const editor = document.createElement("div");
    editor.dataset.autoGrowEditor = "";
    editor.dataset.autoGrowTargetHeight = "48";
    dock.append(editor);

    dock.getBoundingClientRect = () => ({ height: 190 }) as DOMRect;
    editor.getBoundingClientRect = () => ({ height: 120 }) as DOMRect;

    expect(measureComposerDockTargetHeight(dock)).toBe(118);
    expect(measureComposerDockClearance(dock)).toBe(190);
  });
});

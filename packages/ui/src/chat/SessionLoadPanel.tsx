...
export function SessionLoadPanel({ error, language, retrying = false, onRetry, onHome }: {
  ...
  return (
    <div className="flex min-h-0 flex-1 overflow-y-auto px-6 py-12" role={error === undefined ? "status" : "alert"}>
      <div className="m-auto w-full max-w-sm pb-12 text-center">
        ...
        <p className="mx-auto mt-2 max-w-xs whitespace-pre-line text-[13px] leading-6 text-muted-foreground">
        ...
        {error !== undefined && <details className="group relative mt-7 text-left">
          <summary className="mx-auto flex w-fit cursor-pointer list-none items-center gap-1 rounded text-xs text-muted-foreground/75 transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring [&::-webkit-details-marker]:hidden">

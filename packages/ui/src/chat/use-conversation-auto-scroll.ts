...
export function useConversationAutoScroll(
  ...
  { scope = viewportRef, enabled = true }: { scope?: object; enabled?: boolean } = {},
) {
  const following = useRef(true);
  const lastTop = useRef(0);
  const restore = useRef<((resumed?: boolean) => void) | null>(null);
  const enabledRef = useRef(enabled);
  const previous = useRef({ enabled, sessionId });
  ...
  useLayoutEffect(() => {
    const resumed = enabled && !previous.current.enabled && previous.current.sessionId === sessionId;
    previous.current = { enabled, sessionId };
    restore.current?.(resumed);
  }, [viewportRef, sessionId, content, bottomClearance, scope, enabled]);
}

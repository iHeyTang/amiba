 ...
 *
 * hermes-agent's session table records aggregate token counts but not a
 * per-tool-call ledger, so the desktop main process captures the chat
 * engine's `tool.started` / `tool.completed` events into per-day buckets

...
async function loadConfig(): Promise<PersistedConfig> {
  try {
    const r = await getPlatform().storage.get([QUICK_ACTIONS_STORAGE_KEY]);
...
async function saveConfig(next: PersistedConfig): Promise<void> {
  await getPlatform().storage.set({ [QUICK_ACTIONS_STORAGE_KEY]: next });
}
...
export function useQuickActionsController(): QuickActionsController {
  ...
  useEffect(() => {
    ...
    const unsub = getPlatform().storage.watch([QUICK_ACTIONS_STORAGE_KEY], (changes) => {

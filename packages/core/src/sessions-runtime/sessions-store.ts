...
export class SessionsStore {
   ...
   * "all sessions ever" list, backed by Hermes SessionDB). Comparing
   * incoming broadcasts against this lets us swallow our own echo —
   * other windows' edits still flow in.
   *
   * ``activeId`` and ``openTabIds`` are per-window in-memory only;
   * they're not persisted to ``chrome.storage`` and so don't need a
   * hash filter (there's no broadcast to filter).
   */
  ...
  /**
   * Re-fetch the session index from Hermes SessionDB. Called from
   * surfaces that need to pick up multi-channel rows authored elsewhere
   * (gateway, CLI, etc.) without waiting for the next storage-watch
   * broadcast.
   *
  ...
  importSession = async (
    ...
    // Already known locally → just bring the tab to front. Re-seeding
    // the message log would duplicate every row in SessionDB.
    if (this.state.sessions.some((s) => s.id === meta.id)) {
    ...
    const nextSessions = [meta, ...this.state.sessions];
    this.commit({ sessions: nextSessions });
    await this.persistIndex(nextSessions);
    await this.openTab(meta.id);
  };
  ...
  remove = async (id: string): Promise<void> => {
    ...
    // disconnected, and Python ignores the call when the directory
    // doesn't exist.
    try {
      ...
      // Ignore — orphaned files cost only disk.
    }

 ...
 *
 * Caching strategy:
 *   - The fetched record is keyed in the desktop platform store by local
 *     calendar date. As long as a record for today exists we skip the
 *     network entirely, so all renderer surfaces share one fetch per day.
 ...
 *     Electron's HTTP cache handles repeat image loads cheaply, while
 *     persisting base64 image bytes would unnecessarily bloat the profile.
 *
...
function bingMarket(): string {
  // Use the browser's UI language as a heuristic — `navigator.language` is
  // available everywhere (extension renderer, desktop renderer). Falls back
  // to en-US.
  try {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (fresh) {
          setWallpaper(fresh);
          void getPlatform().storage.set({ [CACHE_KEY]: fresh });
        } else if (cached?.url) {
    ...
    // newtab reload.
    const unsub = getPlatform().storage.watch([WALLPAPER_ENABLED_KEY], (changes) => {

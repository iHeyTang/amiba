...
API_TGZ="$ROOT/sdk-dist/amiba-extension-api-${API_VERSION}.tgz"
CLI_TGZ="$ROOT/sdk-dist/amiba-cli-${CLI_VERSION}.tgz"
PRESET_TGZ="$ROOT/sdk-dist/amiba-tailwind-preset-${PRESET_VERSION}.tgz"
for f in "$API_TGZ" "$CLI_TGZ" "$PRESET_TGZ"; do test -f "$f" || { echo "missing $f"; exit 1; }; done

WORK="$(mktemp -d)"
trap 'rm -rf "$WORK"' EXIT
echo "==> scaffolding demo in $WORK"
( cd "$WORK" && printf "\n" | node "$ROOT/apps/cli/dist/cli.js" create demo --id com.example.demo --no-install )
EXT="$WORK/demo"
...
j.devDependencies['@amiba/cli']='file:$CLI_TGZ';
...
echo "==> pack (extension tarball via amiba)"

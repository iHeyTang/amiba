...
export function checkReleaseConfiguration(env) {
  const missing = ['CSC_LINK', 'CSC_KEY_PASSWORD', 'APPLE_ID', 'APPLE_APP_SPECIFIC_PASSWORD', 'APPLE_TEAM_ID'].filter(key => !env[key]?.trim());
  if (missing.length) throw new Error(`Release configuration missing: ${missing.join(', ')}. No version was changed. Configure repository Actions secrets, then run Release again.`);
...
export function prepareRelease({ env, bump = 'patch', directory = root, request = gh, preflight = preflightRelease }) {

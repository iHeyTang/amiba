...
import { verifiedArtifacts, publicArtifactNames, restoreInternalMetadata } from './artifacts.mjs';
...
const names = publicArtifactNames(manifest, verifiedArtifacts(dir, target, pkg.version, process.env.AMIBA_RELEASE_COMMIT));
...
run(['release', 'upload', tag, '--repo', repo, ...names.map(name => path.join(dir, name)), '--clobber']);
if (process.argv.includes('--verify-download')) {
  const verificationDir = fs.mkdtempSync(path.join(os.tmpdir(), 'amiba-release-verify-'));
  try {
    run(['release', 'download', tag, '--repo', repo, '--dir', verificationDir, ...names.flatMap(name => ['--pattern', name])]);
    restoreInternalMetadata(dir, verificationDir, manifest, names);

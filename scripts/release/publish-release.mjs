...
import { verifiedArtifacts, restoreInternalMetadata } from './artifacts.mjs';
...
export function verifyRemoteRelease(repo, version, sets, request = gh) {
  ...
  try {
    ...
    for (const set of sets) {
      const manifest = JSON.parse(fs.readFileSync(path.join(set.dir, 'release-manifest.json')));
      restoreInternalMetadata(set.dir, dir, manifest, set.names);
...
export function publishRelease({ repo, version, commit, names, unsignedMac = false, request = gh, preflight = preflightRelease }) {
  ...
  const installation = unsignedMac ? '## macOS installation\n\nThe macOS packages are unsigned and not notarized. Download the DMG and install manually. macOS may require allowing this app in System Settings > Privacy & Security. Automatic updates are disabled on macOS; install future versions manually.\n\n[Apple installation guidance](https://support.apple.com/102445)\n\n' : '';
  const published = JSON.parse(request(['api', `repos/${repo}/releases/${release.id}`, '--method', 'PATCH', '--input', '-'], JSON.stringify({
    name: `Amiba ${version}`, body: installation + notes.body, draft: false, prerelease: false, make_latest: 'true',
  })));
